...
test(
  ...
  async ({ page }) => {
    ...
    // Protagonista: botó + per afegir seqüència (indica l'acció que segueix)
    ...
    await focusedScreenshot(
      ...
      { padding: 300, mouseOffset: { x: 30, y: 30 } },
    );
    ...
    // Protagonista pas 2: pestanya 2 activa (mostra que s'ha creat una nova seqüència)
    const tab2 = page.locator("#vertical-tab-1");
    await focusedScreenshot(
      page,
      tab2,
      path.join(IMG_DIR, "multiple-sequences-step2.png"),
      { padding: 260, mouseOffset: { x: 25, y: 25 } },
    );

    // Coberta carousel: les dues pestanyes juntes, crop apaïsat per a CardMedia (140px altura)
    // paddingX generós per mostrar context del layout; paddingY reduït per ratio apaïsat
    ...
    // Protagonista: slider de mida global (l'usuari el veu prominentment centrat)
    await focusedScreenshot(
      page,
      sizeSlider,
      path.join(IMG_DIR, "multiple-sequences-step3.png"),
      { padding: 280, mouseOffset: { x: -40, y: 35 } },
    );
    ...
    // =============================================
    // Desactivar el switch "Apply to all" / "Aplicar tots"
    ...
    // Protagonista: slider individual de la seqüència 2
    await focusedScreenshot(
      page,
      seq2SizeSlider,
